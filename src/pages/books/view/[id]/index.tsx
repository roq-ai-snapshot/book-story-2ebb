...
import { useBookFindFirst, useRoqClient } from 'lib/roq';

import { BookInterface } from 'interfaces/book';
import { BookCategoryListPage } from 'pages/book-categories';
...
function BookViewPage() {
  ...
  const queryParams = useMemo<{ include: { publisher: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useBookFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('book', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/books/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem label="Author" text={data?.author} />

                <FormListItem
                  label="Published Date"
                  text={data?.published_date ? format(data?.published_date, 'dd-MM-yyyy') : ''}
                />
                ...
                <FormListItem label="Page Count" text={data?.page_count} />

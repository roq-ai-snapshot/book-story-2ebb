...
type ColumnType = ColumnDef<BookInterface, unknown>;
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['publisher', 'book_category.count', 'review.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['title.contains', 'author.contains', 'genre.contains', 'language.contains'],
          ...params.filters,
        },
        'book',
      ),
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'author', header: 'Author', accessorKey: 'author' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.published_date ? format(record?.published_date, 'dd-MM-yyyy') : '',
    },
    { id: 'genre', header: 'Genre', accessorKey: 'genre' },
    { id: 'language', header: 'Language', accessorKey: 'language' },
    { id: 'page_count', header: 'page_count', accessorKey: 'page_count' },
    hasAccess('publisher', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/publishers/view/${record.publisher?.id}`}>
              {record.publisher?.name}
            </Link>
          ),
        }
      : null,
    hasAccess('book_category', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'book_category',
          header: 'Book Category',
          accessorKey: 'book_category',
          cell: ({ row: { original: record } }: any) => record?._count?.book_category,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/books/view/${record.id}`} passHref legacyBehavior>
                <Button

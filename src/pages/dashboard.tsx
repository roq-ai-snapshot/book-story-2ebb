...
import { useBookCategoryCount, useReviewCount, useBookCount } from 'lib/roq';

import { BookListPage as ShowCaseList } from 'pages/books';

function DashboardPage() {
  const { session } = useSession();

  const bookCategoryStat = useBookCategoryCount();

  const reviewStat = useReviewCount();

  const bookStat = useBookCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Book Categories',
      result: bookCategoryStat?.data,
      isLoading: bookCategoryStat.isLoading,
    ...
    {
      title: 'Books',
      result: bookStat?.data,
      isLoading: bookStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },

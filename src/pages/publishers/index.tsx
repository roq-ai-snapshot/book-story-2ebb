...
import { useRoqClient, usePublisherFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<PublisherInterface, unknown>;
...
export function PublisherListPage(props: PublisherListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'book.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: [
            'description.contains',
            'publisher_status.contains',
            'publisher_type.contains',
            'name.contains',
          ],
          ...params.filters,
        },
  ...
  const { data, error, isLoading, mutate } = usePublisherFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.publisher.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: PublisherInterface) => {
    if (hasAccess('publisher', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/publishers/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'description', header: 'Description', accessorKey: 'description' },
    { id: 'publisher_status', header: 'Publisher Status', accessorKey: 'publisher_status' },
    { id: 'publisher_type', header: 'Publisher Type', accessorKey: 'publisher_type' },
    ...
    hasAccess('book', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Book',
          accessorKey: 'book',
          cell: ({ row: { original: record } }: any) => record?._count?.book,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/publishers/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('publisher', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/publishers/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('publisher', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
